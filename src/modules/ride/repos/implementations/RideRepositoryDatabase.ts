...
export default class RideRepositoryDatabase implements IRideRepository {
  constructor(readonly connection: IConnection) {}

  async save(ride: Ride) {
    await this.connection.query("insert into cccat13.ride (ride_id, passenger_id, from_lat, from_long, to_lat, to_long, status, date) values ($1, $2, $3, $4, $5, $6, $7, $8)", [
      ride.rideId,
      ride.passengerId,
      ride.from.getLat(),
      ride.from.getLong(),
  ...
  async update(ride: Ride): Promise<void> {
    await this.connection.query("update cccat13.ride set driver_id = $1, status = $2, distance = $3, fare = $4 where ride_id = $5", [
      ride.driverId,
      ride.getStatus(),
      ride.getDistance(),
      ride.getFare(),
